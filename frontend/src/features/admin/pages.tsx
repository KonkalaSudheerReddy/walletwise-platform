  ...
  return (
    <div className="space-y-7">
      <PageHeader
        ...
        description="Manage account access without exposing authentication secrets."
        actions={<AdminTabs />}
      />
      <Card className="p-4">
                ...
                <tbody className="divide-y divide-slate-100 dark:divide-slate-800">
                  {users.data.content.map((user) => (
                    <tr key={user.id}>
                      ...
                      <td className="px-5 py-4">{user.preferredCurrency}</td>
                      <td className="px-5 py-4">
...
export function AuditLogsPage() {
  ...
  const update = (name: keyof AuditFilters, value: string | number) =>
    setFilters((current) => ({
      ...current,
      [name]: value || undefined,
      page: name === 'page' ? Number(value) : 0
    }));
  ...
  return (
    <div className="space-y-7">
      <PageHeader
        title="Administration"
        description="Append-only security and business event history."
        actions={<AdminTabs />}
      />
      ...
      {logs.isPending ? (
        ...
        <EmptyState
          ...
          description="Adjust the filters or perform an audited action."
        />
      ) : (
        <>
          <Card className="overflow-hidden">
                ...
                <tbody className="divide-y divide-slate-100 dark:divide-slate-800">
                  {logs.data.content.map((log) => (
                    <tr key={log.id}>
                      ...
                      <td className="px-5 py-4">{log.actorUserId || 'System'}</td>
                      ...
                      <td className="px-5 py-4 font-mono text-xs">
                        {log.correlationId ? `${log.correlationId.slice(0, 12)}...` : 'System'}
                      </td>
                    </tr>
                  ))}
                </tbody>
