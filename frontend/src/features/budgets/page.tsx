...
export function BudgetsPage() {
  ...
  return (
    <div className="space-y-7">
      <PageHeader
        title="Budgets"
        description="Set monthly category limits and see progress before spending becomes a surprise."
        actions={
          <>
            <TextField
              label="Budget month"
      ...
      {budgets.isPending ? (
        ...
        <EmptyState
          ...
          description="Add an expense category limit to begin tracking progress."
          action={
            <Button onClick={() => setEditing('new')}>
        ...
        <section className="grid gap-4 md:grid-cols-2 xl:grid-cols-3" aria-label="Monthly budgets">
          {budgets.data.map((budget) => {
            const tone =
              budget.utilizationPercent > 100
                ? 'danger'
                : budget.utilizationPercent >= budget.alertThresholdPercent
                  ? 'warning'
                  : 'success';
            return (
              <Card key={budget.id} className="p-5">
                ...
                <div className="mt-6">
                  <Progress
                    ...
                    label={`${budget.categoryName} utilization`}
                  />
                </div>
                <div className="mt-5 grid grid-cols-2 gap-3 rounded-xl bg-slate-50 p-4 text-sm dark:bg-slate-950">
                  <div>
                ...
                <div className="mt-4 flex justify-end gap-1">
                  <Button variant="ghost" size="sm" onClick={() => setEditing(budget)}>
      ...
      <Modal
        ...
        title={editing === 'new' ? 'Create budget' : 'Edit budget'}
        description="Budget usage updates after relevant expense activity."
      >
        <BudgetForm
          month={month}
      ...
      <Modal
        ...
        description="Only future or unused budgets can be safely deleted."
      >
