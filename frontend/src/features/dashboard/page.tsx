...
export function DashboardPage() {
  ...
  if (firstError)
    return <ErrorState error={firstError} title="Your dashboard is temporarily unavailable" />;
  if (!analytics.data)
    return (
      <EmptyState
        title="Nothing to analyze yet"
        description="Create a wallet and record your first income to begin."
      />
    );
        ...
        actions={
          <>
            <Link
              ...
              className="inline-flex h-11 items-center gap-2 rounded-xl border border-slate-300 bg-white px-4 text-sm font-medium hover:bg-slate-50 dark:border-slate-700 dark:bg-slate-900"
            >
              <ArrowDownRight className="h-4 w-4 text-rose-600" /> Add expense
            </Link>
            <Link
              to="/transfer"
              className="inline-flex h-11 items-center gap-2 rounded-xl bg-teal-700 px-4 text-sm font-medium text-white hover:bg-teal-800"
            >
        ...
        {[
          {
            ...
            icon: WalletCards,
            tone: 'text-teal-700 bg-teal-50 dark:bg-teal-950'
          },
          {
            label: 'Income',
            value: data.totalIncome,
            icon: ArrowUpRight,
            tone: 'text-emerald-700 bg-emerald-50 dark:bg-emerald-950'
          },
          {
            label: 'Expenses',
            value: data.totalExpense,
            icon: ArrowDownRight,
            tone: 'text-rose-700 bg-rose-50 dark:bg-rose-950'
          },
          {
            label: 'Net cash flow',
            value: data.netCashFlow,
            icon: CreditCard,
            tone: 'text-sky-700 bg-sky-50 dark:bg-sky-950'
          }
      ...
      <section className="grid gap-5 xl:grid-cols-[1.5fr_1fr]">
        <Card className="min-w-0 p-5 sm:p-6">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-sm text-slate-500">Daily expenses this month</p>
            </div>
            <Badge tone="info">{data.transactionCount} entries</Badge>
          </div>
        ...
        <Card className="min-w-0 p-5 sm:p-6">
          <div>
            ...
            <p className="text-sm text-slate-500">Where this month went</p>
          </div>
          {data.expenseByCategory.length ? (
            ...
            <EmptyState
              ...
              description="Add an expense to build a category breakdown."
            />
          )}
        </Card>
      </section>
        ...
        <Card className="p-5 sm:p-6 xl:col-span-2">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-sm text-slate-500">Your latest ledger entries</p>
            </div>
            <Link
          ...
          {!recent.data?.content.length ? (
            <div className="mt-5">
              <EmptyState
                ...
                description="Income, expenses, and transfers will appear here."
              />
            </div>
          ) : (
                ...
                <li key={transaction.id} className="flex items-center justify-between gap-4 py-3">
                  <div className="min-w-0">
                    ...
                    <p className="text-xs text-slate-500">
                      {transaction.walletName} / {formatDate(transaction.occurredAt)}
                    </p>
                  </div>
                  <Money
                    className={
          ...
          {!budgets.data?.length ? (
            <div className="mt-5">
              <EmptyState
                ...
                action={
                  <Link className="text-sm font-medium text-teal-700 hover:underline" to="/budgets">
                    Create a budget
                  </Link>
                }
              />
            ...
            <div className="mt-5 space-y-5">
              {budgets.data.slice(0, 4).map((budget) => (
                <div key={budget.id}>
                  <div className="mb-2 flex justify-between gap-3 text-sm">
                    <span className="font-medium">{budget.categoryName}</span>
                    <span className="text-slate-500">
                      <Money value={budget.spentAmount} currency={data.currency} /> /{' '}
                      <Money value={budget.limitAmount} currency={data.currency} />
                    </span>
                  </div>
                  <Progress
      ...
      <Card className="p-5 sm:p-6">
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-sm text-slate-500">Balances stay separate by currency.</p>
          </div>
          <Link className="text-sm font-medium text-teal-700 hover:underline" to="/wallets">
            Manage wallets
          </Link>
        </div>
        {!wallets.data?.length ? (
          <div className="mt-5">
            <EmptyState
              ...
              description="Opening balances are recorded as immutable ledger entries."
            />
          </div>
        ) : (
          <div className="mt-5 grid gap-3 sm:grid-cols-2 xl:grid-cols-3">
            {wallets.data.slice(0, 3).map((wallet) => (
              <Link
                key={wallet.id}
                to={`/wallets/${wallet.id}`}
                className="rounded-2xl border border-slate-200 p-4 transition hover:border-teal-300 hover:bg-teal-50/40 dark:border-slate-800 dark:hover:border-teal-800"
              >
                <div className="flex items-center justify-between">
