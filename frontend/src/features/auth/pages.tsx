      ...
      <main className="mx-auto grid min-h-[calc(100vh-5rem)] max-w-7xl items-center gap-10 px-5 py-10 sm:px-8 lg:grid-cols-[1fr_460px]">
        <section className="hidden max-w-xl lg:block">
          ...
          <p className="mt-5 text-lg leading-8 text-slate-600 dark:text-slate-300">
            Track virtual balances, build better budget habits, and understand where your money
            goes.
          </p>
          <ul className="mt-8 space-y-3 text-sm text-slate-600 dark:text-slate-300">
            {[
              'Short-lived access tokens stay in memory',
              'Atomic, idempotent wallet transfers',
              'Clear monthly spending analytics'
            ].map((item) => (
...
export function LoginPage() {
  ...
  const submit = async (values: LoginValues) => {
    ...
    try {
      ...
      setFormError(
        error instanceof ApiError ? error.problem.detail : 'Unable to sign in. Please try again.'
      );
    }
  };
    ...
    >
      <form className="space-y-4" onSubmit={(event) => void handleSubmit(submit)(event)} noValidate>
        ...
        <div className="relative">
          ...
          <button
            type="button"
            className="absolute right-2 top-[31px] grid h-9 w-9 place-items-center rounded-lg text-slate-500 hover:bg-slate-100"
        ...
        <p className="text-center text-xs text-slate-500">
          Shared synthetic data may be reset periodically.
        </p>
      </form>
    </AuthenticationCard>
  );
...
export function RegisterPage() {
  ...
  const submit = async (values: RegisterValues) => {
    ...
    try {
      ...
      setFormError(
        error instanceof ApiError ? error.problem.detail : 'Unable to create your account.'
      );
    }
  };
    ...
    >
      <form className="space-y-4" onSubmit={(event) => void handleSubmit(submit)(event)} noValidate>
        ...
        <label className="grid gap-1.5 text-sm font-medium text-slate-700 dark:text-slate-200">
          Preferred currency
          <select
            className="h-11 rounded-xl border border-slate-300 bg-white px-3.5 dark:border-slate-700 dark:bg-slate-950"
...
export function LandingPage() {
  const capabilities = [
    {
      icon: WalletCards,
      title: 'Every wallet in one view',
      description:
        'Organize cash, savings, and virtual bank wallets without connecting to a real institution.'
    },
    {
      icon: SendHorizontal,
      title: 'Retry-safe transfers',
      description:
        'Learn how idempotency and atomic ledger entries protect a transfer from duplicate processing.'
    },
    {
      icon: PiggyBank,
      title: 'Budgets that stay visible',
      description:
        'Track category limits and receive useful alerts as spending approaches a threshold.'
    },
    {
      icon: BarChart3,
      title: 'Monthly financial clarity',
      description: 'Explore income, expenses, category mix, trends, and month-over-month context.'
    }
  ];
  ...
  return (
    <PublicLayout>
      <main>
        <section className="mx-auto grid max-w-7xl items-center gap-14 px-5 py-16 sm:px-8 lg:grid-cols-[1fr_1fr] lg:py-24">
          <div>
            ...
            <p className="mt-6 max-w-xl text-lg leading-8 text-slate-600 dark:text-slate-300">
              WalletWise brings virtual wallets, immutable activity, monthly budgets, and useful
              analytics into one calm workspace.
            </p>
            <div className="mt-8 flex flex-wrap gap-3">
              <Link
                to="/register"
                className="inline-flex h-12 items-center gap-2 rounded-xl bg-teal-700 px-5 font-medium text-white hover:bg-teal-800"
              >
                Create free account <ArrowRight className="h-4 w-4" />
              </Link>
              <Link
                to="/login"
                className="inline-flex h-12 items-center rounded-xl border border-slate-300 bg-white px-5 font-medium hover:bg-slate-50 dark:border-slate-700 dark:bg-slate-900"
              >
                Explore the demo
              </Link>
            </div>
            <p className="mt-5 text-xs text-slate-500">
              Educational portfolio application. No real funds, bank connections, payments, or
              financial advice.
            </p>
          </div>
          ...
          <div
            ...
            aria-label="WalletWise dashboard preview"
          >
            <div className="absolute -inset-5 rounded-[2.5rem] bg-teal-100/70 blur-3xl dark:bg-teal-950/40" />
            <Card className="relative overflow-hidden p-5 sm:p-7">
              <div className="flex items-center justify-between">
                <div>
                  <p className="text-sm text-slate-500">Available in USD</p>
                  <p className="mt-1 text-3xl font-semibold">$12,480.50</p>
                </div>
              ...
              <div className="mt-7 grid grid-cols-2 gap-3">
                <div className="rounded-2xl bg-emerald-50 p-4 dark:bg-emerald-950/40">
                  ...
                  <p className="mt-1 font-semibold">+$5,200.00</p>
                </div>
                <div className="rounded-2xl bg-rose-50 p-4 dark:bg-rose-950/40">
                  ...
                  <p className="mt-1 font-semibold">-$2,740.25</p>
                </div>
              </div>
              <div className="mt-6 flex h-36 items-end gap-2" aria-hidden="true">
                {[32, 48, 38, 72, 51, 84, 63, 92, 58, 76, 66, 88].map((height, index) => (
                  <span
                    key={index}
                    className="flex-1 rounded-t-md bg-teal-600/80"
                    style={{ height: `${height}%` }}
                  />
                ))}
              </div>
          ...
          <div className="mx-auto max-w-7xl px-5 py-20 sm:px-8">
            <div className="max-w-2xl">
              ...
              <h2 className="mt-3 text-3xl font-semibold tracking-tight sm:text-4xl">
                Serious engineering behind a straightforward experience.
              </h2>
            </div>
            <div className="mt-10 grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
              {capabilities.map(({ icon: Icon, title, description }) => (
                ...
                >
                  <span className="grid h-10 w-10 place-items-center rounded-xl bg-teal-100 text-teal-700 dark:bg-teal-950 dark:text-teal-200">
        ...
        <footer className="mx-auto flex max-w-7xl flex-col gap-3 px-5 py-10 text-sm text-slate-500 sm:flex-row sm:items-center sm:justify-between sm:px-8">
          <p>(c) {new Date().getFullYear()} Konkala Sudheer Reddy</p>
          <p>Built as an educational software portfolio project.</p>
        </footer>
      </main>
    </PublicLayout>
  );
