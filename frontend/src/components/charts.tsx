...
const categoryColors = ['#0f766e', '#2563eb', '#d97706', '#db2777', '#7c3aed', '#0891b2', '#65a30d'];
...
export function SpendingTrendChart({ data, currency }: { data: TrendPoint[]; currency: string }) {
  const chartData = data.map((point) => ({ label: point.date.slice(5), amount: amount(point.amount) }));
  return (
    <div className="h-72 min-w-0" aria-label="Daily spending trend chart">
      <ResponsiveContainer width="100%" height="100%">
        <LineChart data={chartData} margin={{ top: 10, right: 10, left: -18, bottom: 0 }}>
          ...
          <Line type="monotone" dataKey="amount" name="Spending" stroke="#0f766e" strokeWidth={3} dot={false} activeDot={{ r: 5 }} />
        </LineChart>
      </ResponsiveContainer>
    </div>
  );
...
export function CategoryDonutChart({ data, currency }: { data: CategoryAggregate[]; currency: string }) {
...
export function IncomeExpenseChart({ income, expense, currency }: { income: DecimalValue; expense: DecimalValue; currency: string }) {
  const chartData = [{ label: 'Selected month', Income: amount(income), Expenses: amount(expense) }];
  return (
